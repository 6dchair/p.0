...
static int RegisterNumber(const char *r) {
    if(r[0] != 'r') 
        return -1;
    int n = atoi(r + 1); // skip first character (w/c is 'r', and directly go to the first digit)
    if(n < 0 || n > 31) 
        return -1;
    return n;
}
...
int MachineFromAssembly(const char *asm_file, const char *out_file) {
    ...
    while(fgets(line, sizeof(line), in)) {
        ...
        if(sscanf(line, "daddiu %7[^,], %7[^,], #%i", r1,r2,&imm) == 3) {
            int rt = RegisterNumber(r1), rs = RegisterNumber(r2); // convert rt and rs strings to reg numbers
            if(rt >= 0 && rs >= 0) { 
